/* AMX_TNT
    a request bomb placement plugin
   
   by CubicVirtuoso
   August 2005
   
   Thanks to: www.games-fusion.net for the c4 model
              C4 model copyright of GamesFusion CS Fusion Pack
              
              core|core|Greenberet on IRC for dynamic permissions
              
              coca for helping me with client_command
   
   This was a requested plugin that I just happened to see on the forums. 
   kungpow333 wanted a TNT plugin where any user could place tnt then 
   explode it as well. The follow plugin does just that. Players can say
   /placetnt to place tnt then /explodetnt to explode tnt. If this was already
   done I'm sorry... I'm still new :) Well enjoy!
   
   Commands:
   say /placetnt : Places TNT
   say /placetnt <time> : Places TNT that will explode in a certain time in secs
   say /explodtnt : Explodes TNT
   amx_tnt_adminonly <1/0> : Enables Admin mode only
   amx_tnt_maxdmg <value> : Sets max damage up to four numbers
   amx_tnt <1/0> : Enable Disable TNT
*/
...
public plugin_init()
{
	...
	register_concmd("say /explodetnt", "CMD_explodetnt", ADMIN_USER)
...
public CMD_placetnt(id, level, cid)
{
	if(functionadmin != ADMIN_ALL && !(get_user_flags(id) & functionadmin))
	{
		client_print(id,print_chat,"TNT Enabled for Admins only")
