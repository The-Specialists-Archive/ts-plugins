/*  AMX Mod X script.
	...
	* amx_loading_flags <flags> (default=abc)
        ...
        the game)
        b - play mp3 on spectate; otherwise stop a track
    ...
    History:
    1.2.6 [2007-03-06]
    + added support for separate config files per map (req. by arkshine)
    1.2.5 [2007-02-15]
...
#endif	
	if(getPCvarAsFlags(g_cvarAmxLoadingFlags) & FL_PLAY_ON_SPECTATE) {
		if(task_exists(id))
			return
...
stock bool:addTrack(const index, const filename[], bool:can_precache=true)
{
    ...
    static sCacheFileName[MAX_TRACKNAME_LENGTH-sizeof(g_sWavDir)+2]
#if defined _DEBUG
    assert 0 <= index < sizeof(g_arrMusicFiles)
#endif
    if(can_precache && (strlen(filename) > g_iWavDirLen) && (filename[g_iWavDirLen] == '\' || filename[g_iWavDirLen] == '/'))
    {   // it's possible that file is placed within precache dir = > check it...
        bPrecache = bool:equali(filename, g_sWavDir, g_iWavDirLen)
        if(bPrecache)
            subStr(sCacheFileName, filename, g_iWavDirLen + 1)
#if defined _DEBUG
        if(bPrecache) 
            log_amx("sCacheFileName = %s", sCacheFileName)
#endif
    }
    
    getFileExt(filename, sExt, sizeof(g_sWavExt))
    ...
    */
    ...
    if(g_arrMusicFiles[index][b_wav] && g_arrMusicFiles[index][b_cached]) {
		copy(g_arrMusicFiles[index][s_filename], MAX_TRACKNAME_LENGTH, sCacheFileName)
    } else {
...
//-----------------------------------------------------------------------------
stock subStr(dest[], const src[], start=0)
{
