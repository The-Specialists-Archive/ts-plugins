////////////////////////////////////////////////////////
// This plugin is made for RDRP by Shin Lee
// The original grabbing code was made by GHW_Chronic
// www.rd-rp.com
////////////////////////////////////////////////////////
	...
	if(is_user_connected(id))
	{
		...
		if(target && is_valid_ent2(target) && target!=id)
		{
			if(target<=maxplayers)
			{
				if(is_user_alive(target))
				{
					client_print(id,print_chat,"[RDRP] Found Target!")
					grabem(id,target)
				}
			}
			else if(get_solidity(target)!=4)
...
public grab_off(id)
{
	if(is_user_connected(id))
	{
		...
		{
			client_print(id,print_chat,"[RDRP] Target Released!")
