	...
	switch(key)
	{
		case 0:
		{
			if(get_pcvar_num(grab_enabled_cvar)==0)
			{
				...
				client_print(0,print_chat,"[RD] Admin has enabled Grab for all clients. Use: bind key +grab")
			}
			else
			{
				set_pcvar_num(grab_enabled_cvar,0)
				client_print(0,print_chat,"[RD] Admin has disabled Grab for all non-admins.")
			}
			show_ghr(id)
		}
		case 1:
		{
			if(get_pcvar_num(hook_enabled_cvar)==0)
			{
				...
				client_print(0,print_chat,"[RD] Admin has enabled Hook for all clients. Use: bind key +hook")
			}
			else
			{
				set_pcvar_num(hook_enabled_cvar,0)
				client_print(0,print_chat,"[RD] Admin has disabled Hook for all non-admins.")
			}
			show_ghr(id)
		}
		case 2:
		{
			if(get_pcvar_num(rope_enabled_cvar)==0)
			{
				...
				client_print(0,print_chat,"[RD] Admin has enabled Rope for all clients. Use: bind key +rope")
			}
			else
			{
				set_pcvar_num(rope_enabled_cvar,0)
				client_print(0,print_chat,"[RD] Admin has disabled Rope for all non-admins.")
			}
			show_ghr(id)
		}
	}
...
public cmd_givetake(id,level,cid)
{
	...
	{
		console_print(id,"[RD] Cannot give/take grab/hook/rope from admin %s.",name)
		return PLUGIN_HANDLED
	}
	...
	if(containi(arg0,"give")!=-1)
	{
		if(containi(arg0,"grab")!=-1)
		{
			if(has_grab[target])
			{
				console_print(id,"[RD] %s already has grab",name)
			}
			else
			{
				has_grab[target]=true
				console_print(id,"[AMXX] %s has been given grab",name)
				client_print(target,print_chat,"[RD] An admin has given you grab. Use: bind key +grab")
			}
		}
		else if(containi(arg0,"hook")!=-1)
		{
			if(has_hook[target])
			{
				console_print(id,"[RD] %s already has hook",name)
			}
			else
			{
				has_hook[target]=true
				console_print(id,"[RD] %s has been given hook",name)
		...
		else if(containi(arg0,"rope")!=-1)
		{
			if(has_rope[target])
			{
				console_print(id,"[RD] %s already has rope",name)
			}
			else
			{
				has_rope[target]=true
				console_print(id,"[RD] %s has been given rope",name)
				client_print(target,print_chat,"[RD] An admin has given you hook. Use: bind key +rope")
			}
		}
	}
	if(containi(arg0,"take")!=-1)
	{
		if(containi(arg0,"grab")!=-1)
		{
			if(!has_grab[target])
			{
				console_print(id,"[RD] %s doesn't have grab",name)
			}
			else
			{
				has_grab[target]=false
				console_print(id,"[RD] %s's grab has been taken away.",name)
				client_print(target,print_chat,"[RD] An admin has taken your grab away.")
			}
		}
		if(containi(arg0,"hook")!=-1)
		{
			if(!has_hook[target])
			{
				console_print(id,"[RD] %s doesn't have hook",name)
			}
			else
			{
				has_hook[target]=false
				console_print(id,"[RD] %s's hook has been taken away.",name)
				client_print(target,print_chat,"[RD] An admin has taken your hook away.")
			}
		}
		if(containi(arg0,"rope")!=-1)
		{
			if(!has_rope[target])
			{
				console_print(id,"[RD] %s doesn't have rope",name)
			}
			else
			{
				has_rope[target]=false
				console_print(id,"[RD] %s's rope has been taken away.",name)
				client_print(target,print_chat,"[RD] An admin has taken your rope away.")
			}
		}
	}
	return PLUGIN_HANDLED
...
public grab_on(id,level,cid)
{
	...
	if(target && is_valid_ent2(target) && target!=id)
	{
		if(target<=maxplayers)
		{
			...
			{
				client_print(id,print_chat,"[RD] Found Target")
				grabem(id,target)
			}
		}
	...
	{
		client_print(id,print_chat,"[RD] Searching...")
...
public grab_off(id)
{
	if(is_user_connected(id))
	{
		if(grab[id]==-1)
		{
			client_print(id,print_chat,"[RD] No Target Found!")
			grab[id]=0
		}
		else if(grab[id])
		{
			client_print(id,print_chat,"[RD] Target Released!")
